...
<div class="form-wrap">
    ...
    <div class="page-header">
        ...
        <span class="status-preview sp-{{ $load->status }}">
            {{ ucfirst(str_replace('_',' ', $load->status)) }}
        </span>
    </div>

    <form method="POST" action="{{ route('loads.update', $load->id) }}" class="form-card">
        @csrf
        @method('PUT')
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <input name="pickup_location" class="form-input"
                           value="{{ $load->pickup_location }}" placeholder="Pickup city">
                </div>
                <div class="form-group">
                    <label class="form-label">Delivery Location</label>
                    <input name="delivery_location" class="form-input"
                           value="{{ $load->delivery_location }}" placeholder="Delivery city">
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="form-grid form-grid-2">
                <div class="form-group">
                    ...
                    <input name="material" class="form-input" value="{{ $load->material }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Weight (Tonnes)</label>
                    <input name="weight" type="number" step="0.1" class="form-input" value="{{ $load->weight }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Price (₹)</label>
                    <input name="price" type="number" class="form-input" value="{{ $load->price }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Expense (₹)</label>
                    <input name="expense" type="number" class="form-input" value="{{ $load->expense }}"
            ...
            <div class="form-grid form-grid-2">
                ...
                <div class="form-group">
                    ...
                    <select name="pickup_time_slot" class="form-select">
                        ...
                        @foreach([
                            ...
                            <option value="{{ $val }}" {{ $load->pickup_time_slot == $val ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="trip_days" min="1" class="form-input"
                           value="{{ $load->trip_days ?? 1 }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Priority</label>
                    <select name="priority" class="form-select">
                        @foreach(['normal','high','low'] as $p)
                            <option value="{{ $p }}" {{ $load->priority == $p ? 'selected' : '' }}>
                                {{ ucfirst($p) }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="form-section">
            ...
            <div class="form-group" style="max-width:260px;">
                ...
                <select name="status" class="form-select">
                    @foreach(['pending','assigned','in_transit','delivered','completed','expired'] as $s)
                        <option value="{{ $s }}" {{ $load->status == $s ? 'selected' : '' }}>
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                <textarea name="notes" class="form-textarea"
                          placeholder="Special instructions or requirements…">{{ $load->notes }}</textarea>
            </div>
        </div>
