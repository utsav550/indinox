...
<div class="page-header">
    <div>
        ...
        <div style="font-size:12px; color:#A8A29E; margin-top:2px; font-family:'IBM Plex Mono',monospace;">
            {{ $loads->count() }} loads found
        </div>
    </div>
    <a href="{{ route('loads.create') }}" class="btn-primary">+ New Load</a>
</div>

{{-- FILTERS --}}
<form method="GET" action="{{ route('loads.index') }}" class="filter-bar">
    <input type="text" name="search" placeholder="Search customer…"
           value="{{ request('search') }}" class="filter-input" style="width:200px;">
    ...
    @if(request('search') || request('status'))
        <a href="{{ route('loads.index') }}" style="font-size:12px; color:#A8A29E; text-decoration:none; padding:7px 10px;">✕ Clear</a>
    @endif
</form>
...
<table class="loads-table">
    ...
    <tbody>
        ...
        <tr class="{{ $priClass }}">
            ...
            {{-- Customer --}}
            <td>
                <div style="font-weight:600;">{{ $load->customer->name ?? '—' }}</div>
                @if($load->customer->company_name ?? false)
                    <div style="font-size:11px; color:#A8A29E;">{{ $load->customer->company_name }}</div>
                @endif
            </td>
            ...
            <td>
                <div class="route-cell">
                    <div class="route-main">{{ $load->pickup_location }} → {{ $load->delivery_location }}</div>
                    <div class="route-sub">{{ $load->material }} · {{ $load->weight }}T</div>
                </div>
            </td>

            {{-- Pickup Date --}}
            <td class="mono" style="white-space:nowrap; color:#57534E;">
                {{ \Carbon\Carbon::parse($load->pickup_date)->format('d M Y') }}
            </td>

            {{-- Truck Type --}}
            <td style="color:#57534E;">{{ $load->truckType->name ?? '—' }}</td>

            {{-- Time Slot --}}
            <td>
                <span class="pill slot-pill">
                    {{ $slotLabels[$load->pickup_time_slot] ?? ucfirst(str_replace('_',' ',$load->pickup_time_slot ?? '—')) }}
                </span>
            </td>

            {{-- Price --}}
            <td class="mono" style="font-weight:600;">
                ₹{{ number_format($load->price) }}
            </td>

            {{-- Priority --}}
            <td>
                <span class="pill pill-{{ $load->priority ?? 'normal' }}">
            ...
            <td>
                <span class="pill {{ $statusPill }}">
                    {{ ucfirst(str_replace('_',' ', $load->status ?? 'pending')) }}
                </span>
            </td>

            {{-- Notes --}}
            <td style="max-width:160px; color:#78716C; font-size:12px;">
                {{ $load->notes ? \Illuminate\Support\Str::limit($load->notes, 40) : '—' }}
            </td>

            {{-- Action --}}
            <td>
                <a href="{{ route('loads.edit', $load->id) }}" class="action-link">Edit</a>
            </td>
